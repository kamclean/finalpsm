...
library(tidyverse); library(mice)
...
Firstly, we want to remove variables we want to ensure the multiple imputation algorithm is based on the appropriate variables. You should define:
 ...
 - `impute` (**Essential**): The variables you want to be imputed. There is signifcant debate whether outcomes should be imputed or not, however outcome is often imputed as best practice as shown below.
...
impdata <- data %>%
  impute(impute = c(var_dep, var_exp), fixed = var_fixed, ignore = var_ignore, m=5)
```

A long dataset is returned containing the original (unimputed) data, with each imputed dataset appended below. Each dataset has a unique number corresponding to it (`m`) and the respective patient (`rowid`).
...
```{r}
source("~/finalpsm/R/finalimp.R")
output <- impdata %>%
  finalimp(dependent = var_dep, explanatory = var_exp, random_effect = NULL)
```
There are 2 outputs:
